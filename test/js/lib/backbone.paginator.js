/* 
	THIS IS A MODIFIED VERSION OF THE LIBRARY, PLEASE CARRY OVER LOCAL CHANGES WHEN UPDATING!
*/
Backbone.Paginator = (function ( Backbone, _, $ ) {
  ...
  Paginator.clientPager = Backbone.Collection.extend({
    ...
    _sortByModule: function (models) {
        if (app.SelectedModule == '#dink' || app.SelectedModule == '#zipvault') {
            var featureModels = [],
            rest = [];
            _.each(models, function (model) {
                if (model.get(app.SelectedModule == '#dink' ? 'IsSignixSigned' : 'HasFiles')) {
                    featureModels.push(model);
                } else {
                    rest.push(model);
                }
            });
            models = featureModels.concat(rest);
        }
        return models;
    },
    ...
    _sort: function (models, sort, direction) {
        // maxv: added ability to customize sort via extended property
        if (this.customSort && typeof this.customSort === "function") {
            return this.customSort(models, sort, direction);
        }
        models.sort(function (a, b) {
            if (sort.indexOf('.') != -1) {
                var attrsClone = _.clone(a.attributes),
                bttrsClone = _.clone(b.attributes),
                sorto = sort.split("."),
                new1 = attrsClone[sorto[0]],
                new2 = bttrsClone[sorto[0]],
                ac = new1[sorto[1]],
                bc = new2[sorto[1]];
            } else {
                var ac = a.get(sort),
      ...
      return this._sortByModule(models);
    },
    ...
    _fieldFilter: function( models, rules ) {
      ...
      _.each(models, function(model){
        ...
        _.each(rules, function(rule){
          ...
          if(rule.type === "function"){
            ...
            // The field's value is required not to be equal to the value in rules.value
      ...
      return this._sortByModule(filteredModels);
    },
    ...
    _filter: function ( models, fields, filter ) {
      ...
      _.each( models, function( model ) {
        ...
        _.each( fields, function( cmp_opts, field ) {
            ...
            var value, arr = field.split(".");
            if (arr.length > 1) {
                value = model.get(arr[0]);
                if (value) value = value[arr[1]];
            } else {
